...
namespace Practice_Linq_2024
{
    internal class Program
    {
        ...
        static void Query1(List<FootballGame> games)
        {
            ...
            var selectedGames =
                from game in games
                where game.Country == "Ukraine" && game.Date.Year == 2012
                select game;
        ...
        static void Query7(List<FootballGame> games)
        {
            ...
            Console.WriteLine(
                $"{g.Date:dd.MM.yyyy} {g.Home_team} - {g.Away_team}, Score: {g.Home_score} - {g.Away_score}, Country: {g.Country}"
            );
        }
        ...
        static void Query8(List<FootballGame> games)
        {
            ...
            foreach (var game in selectedGames)
                Console.WriteLine(
                    $"{game.MatchYear} {game.Team1} - {game.Team2}, Goals: {game.Goals}"
                );
        }
        ...
        static void Query9(List<FootballGame> games)
        {
            ...
            var selectedGames =
                from game in games
                where game.Tournament == "UEFA Nations League" && game.Date.Year == 2023
                select new
                {
                    MatchYear = game.Date.Year,
                    Game = game.Home_team + "-" + game.Away_team,
                    Result = game.Home_score > game.Away_score ? "Win"
                    : game.Home_score < game.Away_score ? "Loss"
                    : "Draw",
                };
        ...
        static void Query10(List<FootballGame> games)
        {
            ...
            var selectedGames = (
                from game in games
                where
                    game.Tournament == "Gold Cup" && game.Date.Year == 2023 && game.Date.Month == 7
                select game
            )
                .Skip(4)
                .Take(6);
